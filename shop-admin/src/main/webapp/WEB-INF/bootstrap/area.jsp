...
<head>
    ...
    <script type="text/html" id="addArea">
    ...
    <script type="text/html" id="updateArea">
        <form class="form-horizontal">
            <div class="form-group">
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $.post(
            "<%=request.getContextPath()%>/user/queryAreaList.do",
            function(data){
        ...
        bootbox.confirm({
            ...
            callback:function (result) {
                if(result){
                    ...
                    if(ids.length>0){
                        $.post(
                            "<%=request.getContextPath()%>/user/deleteAreas.do",
                            {"idList":ids},
                            function(data){
                                if(data.status==200){
    ...
    function add(){
        ...
        $.post(
            "<%=request.getContextPath()%>/user/addArea.do",
            {"name":name,"pid":pid},
            function(data){
                if(data.status==200){
                    //获取被选中的节点集合
                    var nodes = zTreeObj.getSelectedNodes();
                    var newNodes = {"name":name,"pid":pid,"id":data.id};
    ...
    function update(){
        ...
        //先更新数据库  在更新页面节点
        $.post(
            "<%=request.getContextPath()%>/user/updateArea.do",
...
<div id="caozuo" style="display:none;position: fixed ! important; left: 650px; top: 70px;">
